...
namespace RTSEngine
{
	public class CameraMovement : MonoBehaviour {
        ...
        public static CameraMovement instance; //the active instance of this component in the game
        ...
        void Awake ()
        {
            if (instance == null) //only one active instance is allowed per game
                instance = this;
        ...
        void OnMinimapClick ()
        {
            ...
            if (Hits.Length > 0)
            {
                for (int i = 0; i < Hits.Length; i++)
                {
                    ...
                    if (TerrainManager.instance.IsTerrainTile(Hits[i].transform.gameObject))
                    {
                        ...
                        //TO BE CHANGED
                        //if the player presses the right mouse button while selecting his faction's unit(s)
                        else if (Input.GetMouseButtonUp(1) && SelectionMgr.SelectedUnits.Count > 0 && SelectionMgr.SelectedUnits[0].FactionID == GameManager.PlayerFactionID)
                        {
                            
                            //move the selected units to the new clicked position in the minimap
                            MovementManager.instance.Move(SelectionMgr.SelectedUnits,Hits[i].point, 0.0f, null, InputMode.movement, true);
                        }
                    }
                }
            }
