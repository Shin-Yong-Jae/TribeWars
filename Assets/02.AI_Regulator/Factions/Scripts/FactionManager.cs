...
namespace RTSEngine 
{
	public class FactionManager : MonoBehaviour {
		...
		[HideInInspector]
		public List<ResourceCollector> Collectors = new List<ResourceCollector>(); //list containing all the resource collectors that this faction owns.
		[HideInInspector]
		...
		public void AddBuildingToList (Building Building)
		{
            ...
            UpdateLimitsList(Building.GetCode(), true);
		}
		...
		public void RemoveBuilding (Building Building)
		{
            ...
            //update the limits list:
            UpdateLimitsList(Building.GetCode(), false);
		...
		public void AddUnitToLists (Unit Unit)
		{
			...
			if (Unit.gameObject.GetComponent<ResourceCollector> ()) {
				Collectors.Add (Unit.gameObject.GetComponent<ResourceCollector> ());
			}

			//Add the army units (that have the attack component) in one list.
			if (Unit.gameObject.GetComponent<AttackEntity> ()) {
				Army.Add (Unit);
		...
		{
			Units.Remove (Unit);
            UnitManager.allUnits.Remove(Unit);
			...
			//Add the resource gatherers in one list
			if (Unit.gameObject.GetComponent<ResourceCollector> ()) {
				Collectors.Remove (Unit.gameObject.GetComponent<ResourceCollector> ());
			}
        ...
        public void CheckFactionDefeat ()
        {
            ...
            if (GameManager.Instance.GetDefeatCondition() == DefeatConditions.eliminateAll && Units.Count == 0 && Buildings.Count == 0)
                GameManager.Instance.OnFactionDefeated(FactionID);
        }
        ...
        public void CheckCollectorsDropOffBuilding ()
		{
			if (Collectors.Count > 0)
                for (int i = 0; i < Collectors.Count; i++)
                    if (Collectors[i].IsActive()) //check if they are actually currently gathering a resource:
                        Collectors[i].UpdateDropOffBuilding(); //ask them to re-search for the closet drop off building
        }
				...
				for (int i = 0; i < BuildingsList.Length; i++) { //go through the buildings list
					...
					while (j < Buildings.Count && Found == false) { //go through all the faction's buildings.
						if (Buildings [j].IsBuilt == true) {
							if (Buildings [j].GetCode() == BuildingsList [i].GetCode()) { //if we have the same building code.
								Found = true;
							}
						}
						j++;
            ...
            if (Limits.Count > 0) //if there are elements in the building/unit limits list
            {
                ...
                for (int i = 0; i < Limits.Count; i++)
                {
                    ...
                    if (Code == Limits[i].Code)
                    {
                        ...
                        //if the limit has been reached and the player has units selected (
                        if (FactionID == GameManager.PlayerFactionID && Limits[i].CurrentAmount >= Limits[i].MaxAmount && SelectionManager.instance.SelectedUnits.Count > 0)
                            UIManager.instance.UpdateTaskPanel(); //update the task panel
        ...
        public bool CheckRequiredBuildings(Building RefBuilding)
        {
            ...
            foreach (Building.RequiredBuilding BList in RefBuilding.GetRequiredBuildings())
            {
                int i = 0;
                ...
                foreach (Building B in BList.GetBuildingList())
                {
                    RequiredBuildingCodes.Add(B.GetCode());
                }
                ...
                {
                    if (Buildings[i].IsBuilt == true && RequiredBuildingCodes.Contains(Buildings[i].GetCode())) //if the building is built && its code is found in the building list codes
                    {
