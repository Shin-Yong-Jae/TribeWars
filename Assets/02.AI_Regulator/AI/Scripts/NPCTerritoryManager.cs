...
namespace RTSEngine
{
    public class NPCTerritoryManager : NPCComponent
    {
        ...
        void OnBuildingDestroyed (Building building)
        {
            ...
            if(building.FactionID == factionMgr.FactionID && building.BorderComp != null)
            {
                //decrease the current territory ratio.
                UpdateCurrentTerritory(-Mathf.PI * Mathf.Pow(building.BorderComp.Size, 2));
            }
        }
        ...
        {
            currentTerritoryRatio += (value/TerrainManager.instance.GetMapSize()); //update the value.
            if (value > 0) //increase?
            {
