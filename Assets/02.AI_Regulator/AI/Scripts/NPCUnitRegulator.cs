...
namespace RTSEngine
{
    ...
    public class NPCUnitRegulator : NPCRegulator<Unit>
    {
        ...
        {
            return unit.FactionID == factionMgr.FactionID && unit.GetCode() == code || currentInstances.Contains(unit);
        }
        ...
        public void UpdateUnitCreatorList (TaskLauncher taskLauncher, bool add)
        {
            ...
            else if (taskLauncher.TasksList.Count > 0) //if we're adding tasks and this task launcher has tasks.
            {
                ...
                for (int taskID = 0; taskID < taskLauncher.TasksList.Count; taskID++)
                {
                    ...
                    if (taskLauncher.TasksList[taskID].TaskType == TaskManager.TaskTypes.CreateUnit)
                    {
                        //if there are valid prefab(s) assigned to this unit creation task
                        if (taskLauncher.TasksList[taskID].UnitCreationSettings.Prefabs.Count > 0)
                        {
                            string prefabCode = taskLauncher.TasksList[taskID].UnitCreationSettings.Prefabs[0].GetCode();
